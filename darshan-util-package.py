...
class DarshanUtil(AutotoolsPackage):
    ...
    version("dask", branch="snyder/dxt-extra-info-pthread-id", submodules=True)
    ...
    depends_on("autoconf", type="build", when="@dask build_system=autotools")
    depends_on("automake", type="build", when="@dask build_system=autotools")
    depends_on("libtool", type="build", when="@dask build_system=autotools")
